...
r.change.after.extensions.type == "OmsAgentForLinux"
}
condition3(x) if {
r.type == "azurerm_orchestrated_virtual_machine_scale_set"
count({x|r.change.after.extension_profile.extensions[x];condition2(x)}) > 0
}
condition2(x) if {
r.change.after.extension_profile.extensions[x].type == "OmsAgentForLinux"
}